...
public class DumbColourByShooter extends Command {
    ...
    public DumbColourByShooter(LEDsSubsystem leds, ShooterSubsystem shooter) {
        this.leds = leds;
    ...
    public void initialize() {
        leds.trulyStart();
        leds.trulySetBuffer(bufferWhenEmpty);
    }

    @Override
    public void execute() {
            if (shooter.isLoaded()) {
                leds.trulySetBuffer(bufferWhenLoaded);
            } else {
                leds.trulySetBuffer(bufferWhenEmpty);
            }
    }
